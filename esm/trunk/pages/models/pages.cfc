<cfcomponent name="pagesmodel" output="false" extends="resources.abstractmodel">
	...
	<cffunction name="getPagesWEmbeddedcontent">
		<cfargument name="fields" required="true">
		<cfset var g = "">
		<cfset var itm = "">
		<cfquery name="g" datasource="#variables.requestObj.getvar('dsn')#">
			SELECT distinct '/' + pp.urlpath path, pp.id, po.id poid
			FROM stagedpages pp
			INNER JOIN pageObjects_view po ON po.pageid = pp.id AND po.status = 'staged' AND po.deleted = 0 AND po.siteid = <cfqueryparam value="#variables.userObj.getCurrentSiteId()#" cfsqltype="cf_sql_varchar">
			WHERE  1 = 1 
				<cfloop collection="#fields#" item="itm">
					AND data LIKE <cfqueryparam value="%#fields[itm]#%" cfsqltype="cf_sql_varchar">
				</cfloop>
				AND pp.siteid = <cfqueryparam value="#variables.userObj.getCurrentSiteId()#:staged" cfsqltype="cf_sql_varchar">
		</cfquery>

		<cfreturn g>
	</cffunction>
	
	<cffunction name="urlMatch" output="false">
		<cfargument name="criteria" required="true">
		<cfset var g = "">
		<!--- determine if user is allowed to root --->
		<!--- <cfset var rootallowed =  variables.userobj.isPathAllowed('')>
		...
		<cfquery name="g" datasource="#variables.requestObj.getvar('dsn')#">
			SELECT '/' + urlpath path
			FROM sitepages_view
				WHERE  urlpath LIKE  <cfqueryparam value="#rereplace(arguments.criteria,"^/","")#%" list="false" cfsqltype="cf_sql_varchar">
				AND siteid = <cfqueryparam value="#variables.userObj.getCurrentSiteId()#:staged" cfsqltype="cf_sql_varchar">
		</cfquery>
