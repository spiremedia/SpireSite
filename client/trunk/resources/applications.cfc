<cfcomponent name="modules">
	...
	<cffunction name="addapp">
        ...
        <cfloop from="1" to="#arraylen(processes)#" index="i">
			...
			<cfloop collection="#prs.xmlattributes#" item="ii">
				<cfset info[ii] = prs.xmlattributes[ii]>
			</cfloop>
			
			<cfset info.parameterlist = structnew()>
			<cfif structkeyexists(prs.xmlattributes, "parameterList")>
	...
	<cffunction name="search">
		...
		<cfloop from="1" to="#arraylen(variables.appactions)#" index="i">
			<cfif refindnocase(variables.appactions[i].match, path)>
				<cfreturn duplicate(variables.appactions[i])>
			</cfif>
		</cfloop>
		<cfreturn structnew()>
	</cffunction>
	...
	<cffunction name="dump">
		dump in applications
		<cfdump var=#variables.appactions#>
		<cfabort>
	</cffunction>
</cfcomponent>