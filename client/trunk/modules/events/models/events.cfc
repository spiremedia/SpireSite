<cfcomponent name="Events" extends="resources.abstractModel">
	...
	<cffunction name="getEventsList">
    	<cfargument required="no" name="sortby" default="startdate">
		<cfset var me = "">
        <cfif not listfindnocase("startdate,location",arguments.sortby)> 
        	<cfset arguments.sortby = "startdate">
        </cfif>
		<cfquery name="me" datasource="#variables.requestObject.getVar('dsn')#">
			SELECT name, id, title, locationname, location, startdate, enddate, shortdescription, description
			FROM events_view
	...
	<cffunction name="getEvent">
		...
		<cfset var me = "">
		<cfquery name="me" datasource="#variables.requestObject.getVar('dsn')#">
			SELECT e.id, e.title, e.locationname, e.location, e.startdate, e.enddate, e.starttime, e.endtime, imageassetid, 
			e.shortdescription, e.description, e.maplink, e.showmaterialsform, e.showaddtlattendees,
			 a.name, a.filesize, a.filename, a.id AS asset_id
			FROM events_view e
	...
	<cffunction name="getHomePageItems">
		<cfset var me = "">
		<cfquery name="me" datasource="#variables.requestObject.getVar('dsn')#">
			SELECT id, title, startdate, enddate, locationname, location
			FROM events_view
			WHERE enddate IS NULL OR enddate > getDate() 
			AND onhomepage = 1 
			AND active = 1
	...
	<cffunction name="registrationValidate">
        ...
        <cfset vdtr.maxlength('locationname',75,info.locationname, 'Location Name may not be longer than 75 chars')>
        <cfset vdtr.maxlength('shortdescription',150,info.shortdescription, 'Short Description may not be longer than 150 chars')>
...
Please refer back to <a href="#requestObject.getVar('siteurl')#NewsAndEvents/Events/#evtinfo.id#/">#evtinfo.title#</a> for details about the event.<br><br>
		</cfsavecontent>
		</cfoutput>
