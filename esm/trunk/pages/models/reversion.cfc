<cfcomponent name="revision" extends="resources.abstractModel">
	...
	<cffunction name="getRevertibleItem">
		...
		<cfquery name="r" datasource="#variables.requestObject.getvar('dsn')#">
			SELECT 
				ppoa.name, ppoa.[module], ppoa.membertype, ppoa.id, ppoa.data, ppoa.title,
	...
	<cffunction name="revertItem">
		...
		<cfset lcl.revertibleItem = getRevertibleItem(variables.requestObject.getFormUrlVar('revertibleId'))>
		...
		<cfquery name="lcl.revertpo" datasource="#variables.requestObject.getvar('dsn')#">
			INSERT INTO [pageObjects](
		     ...
		     VALUES
		           ...
		           ,<cfqueryparam value="#lcl.revertibleItem.title#" cfsqltype="cf_sql_varchar">
	...
	<cffunction name="getRevertableTargets">
		...
		<cfloop query="viewlist">
			<cfif structkeyexists(viewlist.parameterlist[currentrow],'editable') 
					...
					AND listfind(viewlist.modulename, module)>
				<cfset s = structnew()>
		...
		<!--- <cfloop from="1" to="#arraylen(viewlist.data.parameterlist)#" index="l.item">

			<cfif structkeyexists(viewlist.data.parameterlist[l.item],'editable') 
					AND viewlist.data.parameterlist[l.item]['editable']
					AND listfind(viewlist.data.modulename[l.item], arguments.module)>

				<cfset s = structnew()>
				<cfset s.name = viewlist.data.name[l.item]>
				<cfset s.membertype = "default">
				<cfset arrayappend(alist, s)>
				<cfif structkeyexists(pos, viewlist.data.name[l.item]) AND structkeyexists(pos[s.name],'default')>
					<cfloop query="memberTypes">
						<cfset s = structnew()>
						<cfset s.name = s.name>
						<cfset s.membertype = memberTypes.name>
		...
		<cfloop query="viewlist">
			...
			<cfif structkeyexists(parameterlist,'editable') 
					AND parameterlist['editable']
					AND listfindnocase(modulename, arguments.module)>

				<cfset s = structnew()>
				<cfset s.name = viewlist.name>
				<cfset s.membertype = "default">
				<cfset arrayappend(alist, s)>
				<!--- functionality for assigning content areas to member types was changed 
				so the below is not the default behavior and problematic for clients who use membertypes solely for page restrictions
				If client does have content areas targeted at specific member types, comment this back in --->
				<!--- <cfif structkeyexists(pos, name) AND structkeyexists(pos[s.name],'default')>
					<cfloop query="memberTypes">
						<cfset s = structnew()>
						<cfset s.name = viewlist.name>
						<cfset s.membertype = memberTypes.name>
