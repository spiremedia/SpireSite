<cfcomponent  
	...
	<cffunction name="APDateFormat" returntype="string" hint='changes the format of dates to match Associated Press Style'>
		<cfargument name="dateIn" type="string" required=true>
		<!--- Correctly abbreviates months in the date to match AP Style rules. 
			Those are: "Jan." "Feb." "March" "April" "May" "June" "July" "Aug." "Sept." "Oct." "Nov." "Dec."
		 --->
		<cfif NOT isDate(arguments.dateIn)>
			<cfreturn ''>
		<cfelseif ListFind("1,2,8,10,11,12",month(arguments.dateIn))>
			<cfreturn dateformat(arguments.dateIn,"mmm. dd, yyyy")>
		<cfelseif month(arguments.dateIn) eq 9>
			<cfreturn 'Sept. #dateformat(arguments.dateIn,"dd, yyyy")#'>
		<cfelse>
			<cfreturn dateformat(arguments.dateIn,"mmmm dd, yyyy")>
		</cfif>
	</cffunction>
