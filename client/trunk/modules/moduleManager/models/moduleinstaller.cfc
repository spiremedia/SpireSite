<cfcomponent name="Module Installer" output="false" extends="resources.abstractmodel">
	...
	<cffunction name="createDirs">
		<cfdirectory action="create" directory="#requestObj.getVar("machineroot") & "/modules/" & variables.info.module#" mode="664">
		<cfset notes.note('Created dir "#variables.info.module#".')>
		<cfdirectory action="create" directory="#requestObj.getVar("machineroot") & "/modules/" & variables.info.module#/work" mode="664">
	...
	<cffunction name="downloadziptowork">
		...
		<cfhttp
		    method="Get"
		    url="http://esm3installer.spiremedia.com/#variables.info.client_zip#"
		    path="#requestObj.getVar("machineroot") & "/modules/" & variables.info.module & "/work"#"
		    file="modulezip.zip">
	...
	<cffunction name="checksafe">
		...
		<cfzip  
			action = "list"
		    	file = "#requestObj.getVar("machineroot") & "/modules/" & variables.info.module & "/work/modulezip.zip"#"
		    		name = "l.zipcontents"
		    			recurse = "yes">
			<cfcatch>
	...
	<cffunction name="extractzipfiles">
		<cfzip
		    action = "unzip"
		    destination = "#requestObj.getVar("machineroot") & "/modules/" & variables.info.module#"
		    file = "#requestObj.getVar("machineroot") & "/modules/" & variables.info.module & "/work/modulezip.zip"#"
		    overwrite = "yes"
		    recurse = "yes"
		    storePath = "yes">
	...
	<cffunction name="localinstall">
		<cfset l = structnew()>
		<cfif fileexists("#requestObj.getVar("machineroot") & "/modules/" & variables.info.module & "/install/installer.cfc"#")>
			<cfset notes.note('Module installer found.')>
			<cfset l.inst = createObject("component","modules.#variables.info.module#.install.installer").init(requestObj)>
	...
	<cffunction name="cleanup">
		<cfset var l = structnew()>
		<cfdirectory action="list" name="l.list" directory="#requestObj.getVar("machineroot") & "/modules/" & variables.info.module#/work">
		
		<cfloop query="l.list">
			<cffile action="delete" file="#l.list.directory#/#l.list.name#">
		</cfloop>
		
		<cfdirectory action="delete" directory="#requestObj.getVar("machineroot") & "/modules/" & variables.info.module#/work">
		...
		<cfelse>
			<cfdirectory action="list" name="l.list" directory="#requestObj.getVar("machineroot") & "/modules/" & variables.info.module#/install">
			...
			<cfif directoryexists("#requestObj.getVar("machineroot") & variables.info.module#/install")>
				<cfdirectory action="delete" directory="#requestObj.getVar("machineroot") & "/modules/" & variables.info.module#/install">
			</cfif>
			<cfset notes.note('Deleted Install directory.')>
		</cfif>
		
		<cfset notes.note('Completed cleanup. Done!')>
		<cfset notes.note('Finish with a ?refresh and then a unittest.')>
