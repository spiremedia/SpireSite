	...
	$.fn.galleriffic = function(thumbsContainerSel, settings) {
		...
		$.extend(this, {
			...
			buildPager: function(pager) {
				...
				// Prev Page Link
				if (this.currentPage > 0 && this.settings.enablePrevNextButtons) {
					var prevPage = startIndex - this.settings.numThumbs;
					pager.append('<a class="pageingPrevNext" rel="history" href="#'+this.data[prevPage].hash+'" title="Previous">'+this.settings.prevPageLinkText+'</a>');
				}
				...
				if (nextPage < this.data.length && this.settings.enablePrevNextButtons) {
					pager.append('<a class="pageingPrevNext" rel="history" href="#'+this.data[nextPage].hash+'" title="Next">'+this.settings.nextPageLinkText+'</a>');
				}
		...
		if (this.settings.controlsContainerSel) {
			...
			if (this.settings.renderNavControls) {
				var $navControls = this.$controlsContainer
					.append('<div class="nav-controls"><a class="prev" rel="history" title="Previous Image">'+this.settings.prevLinkText+'</a><a class="next" rel="history" title="Next Image">'+this.settings.nextLinkText+'</a></div>')
