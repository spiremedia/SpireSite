<cfcomponent displayname="eventTest" extends="mxunit.framework.TestCase">
	...
	<cffunction name="teardown">
		<cfquery name="me" datasource="#request.requestObject.getVar('dsn')#">
			DELETE FROM events WHERE name = 'unittesting'
		</cfquery>
    ...
    <cffunction name="InterfaceTest">
		...
		<cfset variables.httpObj.setPath('/Events/startPage/')>
		<cfset response = variables.httpObj.load()>
		
		<cfset assertfalse(condition=(response.is302relocate()),message="sa logged in but can't see events")>
		
	<!--- testing add events --->
		<cfset variables.httpObj.setPath('/Events/AddEvent/')>
		<cfset response = variables.httpObj.load()>
		
		<cfset l.formFields = response.getESMFormFields()>

		<cfset assertfalse(condition=(len(l.formFields) EQ 0),message="no form fields in add events page")>
		
	<!--- testing save new events --->
		...
		<cfset response = variables.httpObj.load()>
		<cfset asserttrue(condition=(response.getStatus() EQ '200'),message="error while saving new event")>
		...
		<cfset asserttrue(condition=(response.existsByPattern("""FIELD"":""startdate""")),message="validation did not find startdate error")>
		<cfset asserttrue(condition=(response.existsByPattern("""FIELD"":""enddate""")),message="validation did not find enddate error")>
	...
	<!--- test edit events form--->
		<cfset l.id = response.getByPattern('[a-zA-Z0-9\-]{35}')>
		<cfset l.fields['id'] = l.id>
		
		<cfset variables.httpObj.setPath("/Events/EditEvent/")>
		...
		<cfset asserttrue(condition=(response.getStatus() EQ '200'),message="error while getting edit event form")>

	<!--- test save existing event --->
		...
		<cfset assertEquals(expected = l.fieldsOut, actual = l.fields, message="When saving event, query in is not query out")>
		...
		<cfset variables.httpObj.setPath("/Events/saveEvent/")>
		
		<cfset response = variables.httpObj.load()>

		<cfset asserttrue(condition=(response.getStatus() EQ '200'),message="error while updating event")>
	...
	<!--- test deleting event --->
		...
		<cfset variables.httpObj.setPath("/Events/deleteEvent/")>
		<cfset variables.httpObj.addUrlField('id', l.id)>
		
		<cfset response = variables.httpObj.load()>

		<cfset asserttrue(condition=(response.getStatus() EQ '200'),message="error while deleting event")>
		...
		<cfset asserttrue(condition=(l.findid EQ ""),message="event was not deleted")>
	</cffunction>
