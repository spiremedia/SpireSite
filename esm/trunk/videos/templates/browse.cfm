...
<cfsilent>
	...
	<!--- <cfoutput query="lcl.list" group="Videogroupname"> --->
		<cfset lcl.count = lcl.count + 1>
		<cfsavecontent variable="lcl.s.html">
			<div class="nav">
			<ul>
			<cfoutput query="lcl.list">
				<li><a <cfif lcl.id EQ lcl.list.id[lcl.list.currentrow]>class="selected" <cfset lcl.acc.setselected(lcl.count)></cfif> href="../editVideo/?id=#id#">#name#</a></li>
			</cfoutput>
			</ul>
			</div>
		</cfsavecontent>
		<cfset lcl.acc.add("Videos",lcl.s.html)>
	<!--- </cfoutput> --->
	
	<cfelse>
		<cfset lcl.acc.add("No Videos Loaded","")>
	</cfif>
</cfsilent>
