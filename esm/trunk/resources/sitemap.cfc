<cfcomponent name="sitemap" hint="Tree cfc" output="false">
	...
	<cffunction access="public"  name="moveToFolder">
		<cfargument name="idtomove" required="yes" type="uuid">
		<cfargument name="targetparentid" required="yes" type="uuid">
	...
	<cffunction access="public" name="Moveupdown" output="false">
		...
		<!--- 10/28/2009 updated to set the sort to be the same on both staged and published pages as they were getting out of sync with one another --->
		<cfquery name="updateoriginal" datasource="#variables.dsn#" result="n">
			...
			WHERE id = <cfqueryparam value="#switchinfo.currentid#" cfsqltype="CF_SQL_VARCHAR">
				<!--- AND siteid = <cfqueryparam value="#variables.siteid#" cfsqltype="CF_SQL_VARCHAR"> --->
				AND siteid LIKE <cfqueryparam value="#listfirst(variables.siteid,':')#%" cfsqltype="CF_SQL_VARCHAR">
		</cfquery>
		
		<!--- 10/28/2009 updated to set the sort to be the same on both staged and published pages as they were getting out of sync with one another --->
		<cfquery name="updateswitchwith" datasource="#variables.dsn#" result="o">
			UPDATE #variables.tablename#
