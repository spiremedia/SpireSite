                ...
                <cfif datecompare(datemade, now()) is 1 AND NOT structkeyexists(url, "reset")>
                    ...
                    <cfloop from="1" to="#arraylen(attributes.postProcess)#" index="i">
                    	<cfset obj = createObject('component', 'modules.#attributes.postProcess[i].folder#.controller').init(		
								IIF(structkeyexists(attributes.postProcess[i].parameterList, 'title'), "attributes.postProcess[i].parameterList.title", DE("")),								
								structnew(), 
								attributes.requestObject, 
								attributes.postProcess[i].parameterList, 
								"")>
						<cfif structkeyexists(attributes.postProcess[i], "action")>
					        <cfinvoke component="#obj#" method="#attributes.postProcess[i].action#" returnvariable="obj">
					       		<cfinvokeargument name="module" value="#attributes.postProcess[i].folder#">
					       		<cfinvokeargument name="moduleaction" value="#attributes.postProcess[i].action#">
					       	</cfinvoke>
				       	</cfif>
                        <cfset html = replacenocase(html, 
													attributes.postProcess[i].replacestring, 
													obj.showHTML(attributes.postProcess[i].folder, attributes.postProcess[i].action))>
                    </cfloop>
                    <cfoutput>
						<cfset cached = true><cfcontent reset="yes">#html#</cfoutput>
                    <cfexit>
                </cfif>
            </cfif>
        </cflock>
...
<cfif thistag.executionmode EQ 'end'>
	...
	<cfloop from="1" to="#arraylen(attributes.postProcess)#" index="i">
    	<cfif findnocase(attributes.postProcess[i].replacestring, thistag.generatedContent)>
	        ...
	        <cfset thistag.generatedcontent = replacenocase(thistag.generatedcontent, attributes.postProcess[i].replacestring, obj.showHTML(attributes.postProcess[i].folder, attributes.postProcess[i].action ))>
        </cfif>
    </cfloop>
</cfif>