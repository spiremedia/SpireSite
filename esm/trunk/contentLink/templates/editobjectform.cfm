...
<cfoutput>
    <cfif items.getDefault.recordcount>
        ...
        <a href="/#items.getDefault.module#/editClientModule/?id=#items.getDefault.id#">Default</a> (#items.getDefault.module#).
        </p>
    ...
    <cfif items.nonDefault.recordcount>
        ...
        <select name="alternate" onChange="if (this.value != '') location.href=this.value;">
        <option value="">Choose</option>
        <cfloop query="items.nonDefault">
            <option value="/#items.nonDefault.module#/editClientModule/?id=#items.nonDefault.id#">#items.nonDefault.memberType# (#items.nondefault.module#)</option>
        </cfloop>
        </select>
        </p>
        <hr size="1" noshade width="60%" align="left" />
    ...
    <cfif items.unusedgroups.recordcount>
        ...
        <cfset lcl.form.addformitem('pageid', '', false, 'hidden', requestObj.getFormUrlVar("pageid"))>
        <cfset lcl.form.addformitem('objName', '', false, 'hidden', requestObj.getFormUrlVar("name"))>
