<cfcomponent name="http" output="false">
	...
	<cffunction name="getESMFormFieldValue" output="false">
		...
		<cfset var l = structnew()>
		<cfset json = createObject("component", "utilities.json")>
		<cfset l.html = getHTML()>
		<cfset l.val = "">
		<!--- find all hidden, text, input fields --->
		...
		<!--- list manager --->
		<cfset l.formA = reMatchNoCase("new ListManager\('#field#',(\[[^\]]+\])", l.html)>

		<cfloop array="#l.formA#" index="l.str">
			<cfset l.re = refindnocase("\[[^\]]+\]", l.str, 1, true)>
			<cfset l.tmp = json.decode(mid(l.str, l.re.pos[1], l.re.len[1] ))>
			<cfset l.val =  listappend(l.val, arraytolist(l.tmp))>
		</cfloop>
	...
	<cffunction name="getESMSubmitsTo" output="false">
		...
		<cfset rl.findinfo = refindnocase("ajaxWResponseJsCaller\('([^\']+)',formStr\);", l.html, 1, true)>
	...
	<cffunction name="existsByPattern"  output="false">
		<cfargument name="re">
		<cfreturn refindnocase(re, rereplace(getHTML(), "[\n\r\t]", "", "all"))>
	</cffunction>
	
	<cffunction name="getByPattern" output="false">
		<cfargument name="re" required="true">
		<cfargument name="item" default="1">
		
		<cfset var l = structnew()>
		
		<cfset l.html = rereplace( getHTML(), "[\n\r\t]", "", "all")>
		
		<cfset l.findinfo = refindnocase(re, l.html, 1, true)>
		
		<cfif l.findinfo.pos[item] EQ 0>
			<cfreturn "">
		</cfif>
		
		<cfreturn mid(l.html, l.findinfo.pos[item], l.findinfo.len[item])>
	</cffunction>
