<cfcomponent name="formatter" extends="utilities.esmmenusiloformatter">
	...
	<cffunction name="getHTML" output="true">
		...
		<cfset var selectedsiloid = "">
		<cfset var selectedparents = "">
		<cfset var formatter = "">
		<cfset var silohtml = "">
		...
		<!--- figure out silo id --->
		<cfif variables.selectedid NEQ "">
			<cfset selectedparents = variables.siteMapObject.getparents(variables.selectedid)>
			<cfif selectedparents.recordcount LTE 1>
				<cfset selectedsiloid = variables.selectedid>
			<cfelse>

				<cfset selectedsiloid = selectedparents.id[selectedparents.recordcount - 1]>
			</cfif>
		</cfif>
		
		<!--- setup the home page accordion area --->
		<cfset link = "<a id='#tli.id#' href='/pages/editPage/?id=#tli.id#'>#reReplace(tli.pagename, "<(.|\n)*?>", "", "ALL")#</a>">
		...
		<cfloop query="tlis">
			...
			<cfset more.id = tlis.id>
			<cfset more.class = "empty">
			
			<cfset silohtml = "">
			<cfif selectedsiloid EQ tlis.id>
				<cfset silohtml = getsilo(tlis.id, variables.selectedid, allowedLocationIds, variables.userobject, variables.stateobject, variables.sitemapobject)>
				<cfset more.class = "">
			</cfif>
			...
			<cfif silohtml NEQ "" AND NOT rootallowed AND NOT userObject.isPathAllowed(tlis.urlpath)>
			...
			<cfset accordionObject.add(tlis.pagename, silohtml, more)>
