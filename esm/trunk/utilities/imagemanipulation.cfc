<cfcomponent name="asset model" output="false" extends="resources.abstractmodel">
	...
	<cffunction name="resizetomax">
		...
		<!--- no resize/rename necessary --->
		<cfif (imginfo.w eq arguments.maxw) AND (imginfo.h eq arguments.maxh) AND (lcase(arguments.imgfile) eq lcase(arguments.newimgfile))>
			<cfset local2.w = imginfo.w>
			<cfset local2.h = imginfo.h>
			<cfset local2.found = 1>
			<cfreturn local2>
		</cfif>
