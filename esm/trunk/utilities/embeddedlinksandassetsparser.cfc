<cfcomponent name="model" output="false" extends="resources.abstractContentObjectEditorModel">
	...
	<cffunction name="preprocessforwysywig">
        ...
        <!---
			do the same with links which come in as {{link[uuid]}}
		--->
       	<cfset lcl.linkre = "href=""{{link\[([A-Z0-9\-]{35})\]\[([A-Z0-9\-]{35})\]}}""">
        <cfset lcl.refinfo = refindnocase(lcl.linkre, arguments.content, 0, true)>
		<cfset lcl.count = 0>
	  	<cfloop condition="arraylen(lcl.refinfo.len) EQ 3 AND lcl.count LT 200">

	  		<cfset lcl.sid = mid(arguments.content, lcl.refinfo.pos[2], lcl.refinfo.len[2])>
            <cfset lcl.id = mid(arguments.content, lcl.refinfo.pos[3], lcl.refinfo.len[3])>
	  		
	  		<cfset lcl.fullstring = mid(arguments.content, lcl.refinfo.pos[1], lcl.refinfo.len[1])>
	  		<cfset lcl.newstring = "href=""/esmtemplink/?sid=#lcl.sid#&id=#lcl.id#""">
	...
	<cffunction name="postprocessfromwysywig">
        ...
        <!---
			same confusion with link
			href=""/esmtemplink/?sid=#lcl.id#&id=#lcl.id#"
		--->

        <cfset lcl.linkre = "href=""\/esmtemplink\/\?sid=([A-Z0-9\-]{35})(&|&amp;)id=([A-Z0-9\-]{35})""">
        <cfset lcl.refinfo = refindnocase(lcl.linkre, arguments.content, 0, true)>
		<cfset lcl.count = 0>
	  	<cfloop condition="arraylen(lcl.refinfo.len) EQ 4 AND lcl.count LT 200">

	  		<cfset lcl.sid = mid(arguments.content, lcl.refinfo.pos[2], lcl.refinfo.len[2])>
            <cfset lcl.id = mid(arguments.content, lcl.refinfo.pos[4], lcl.refinfo.len[4])>
	  		
	  		<cfset lcl.fullstring = mid(arguments.content, lcl.refinfo.pos[1], lcl.refinfo.len[1])>
	  		<cfset lcl.newstring = "href=""{{link[#lcl.sid#][#lcl.id#]}}""">
		...
		<!--- rewrite table widths --->
		<cfset refinfo = refind("<table[^>]+(width=""[0-9]+"")([^>]+)?>", arguments.content, 0, true)>
      
		<!---<cfdump var=#arguments.content#>
      	<br><br>--->
		<cfloop condition="arraylen(refinfo.len) GTE 2">
			<cfset width = mid(arguments.content, refinfo.pos[2] + 7, refinfo.len[2]-8)>
			<!---<cfdump var=#width#><br><br>--->
			<cfset tempNew = temp>
			<cfset strStyle = 'style="width:#width#px"'>

			<cfset arguments.content = mid(arguments.content, 1, refinfo.pos[2]-1) & strStyle & mid(arguments.content,  refinfo.pos[2] + refinfo.len[2], len(arguments.content))>
			<cfset refinfo = refind("<table[^>]+(width=""[0-9]+"")([^>]+)?>", arguments.content, refinfo.pos[1] + 1, true)>
		</cfloop>
