<cfcomponent name="page" output="false" extends="resources.abstractmodel">
	...
	<cffunction name="getPageName" output="false">
		<cfreturn variables.itemdata.pagename>
	</cffunction>
	
	<cffunction name="getUrlPath" output="false">
	...
	<cffunction name="validate">
		...
		<cfset vdtr.lengthbetween('template', 1,50, requestvars.template, 'The Template must be selected.')>
		<cfset vdtr.regexmatchtest('pageurl', '[^a-zA-Z0-9\-]+', requestvars.pageurl, 'The Page URL should only contain letters, digits, or hyphens. No spaces please.')>
		
		<!-- check length -->
		<cfset vdtr.lengthbetween('Title',  1,500,requestvars.title, 'The Browser Title is required and should be less than 500 chars.')> 
		<cfset vdtr.maxlength('description', 500, requestvars.description, 'The Meta Data Description should be less than 500 chars ')> 
		<cfset vdtr.maxlength('keywords', 500, requestvars.keywords, 'The Meta Data Keywords should be less than 500 chars ')> 
		<cfset vdtr.maxlength('summary', 255, requestvars.summary, 'The Meta Data Search Results Description should be less than 255 chars ')> 
		
		<cfif requestvars.id EQ requestvars.parentid>
			<cfset vdtr.addError('parentid', 'The Parent Page may not be this page.')>
		</cfif>		
		...
		<cfif requestvars.parentidold neq requestvars.parentid>
			<cfif not (userobj.isSuper() OR userobj.isPathAllowed(""))>
				...
				<cfif not variables.userobj.isDescendentPathAllowed(newparentpath)>
					<cfset vdtr.addError('parent', 'The page selected in the Parent field is not permitted for this page. This can occur if you try to set the page to be its own parent or to be the home page or if you do not have privileges to that page.  Please select a different Parent page.')>
				</cfif>
			</cfif>
		</cfif>
	...
	<cffunction name="save">
		...
		<cfif variables.itemData.id EQ ''>
			...
			<cfset variables.request.notifyObservers("orm.save.sitepages", this)>
			<cfreturn id>
		<cfelse>
		...
		<cfif links.recordcount>
			<cfif links.recordcount GT 1>
                ...
                <cfif links.siteid NEQ userObj.getCurrentSiteId()>
                    <cfset errorstring = errorstring & "&nbsp;&nbsp;-On site #sitename# on pagewith path ""/#iif(links.urlpath EQ "", DE('Home'),DE(links.urlpath))#""<br>">
                <cfelse>
                    <cfset errorstring = errorstring & "&nbsp;&nbsp;-On page with path ""/#iif(links.urlpath EQ "", DE('Home'),DE(links.urlpath))#.""<br>">
                </cfif>
                </cfoutput>
            <cfelseif links.recordcount EQ 1>
                <cfif links.siteid NEQ userObj.getCurrentSiteId()>
                    <cfset errorstring = "This page is linked from site #links.sitename# on pagewith path ""/#iif(links.urlpath EQ "", DE('Home'),DE(links.urlpath))#""<br>">
                <cfelse>
                    <cfset errorstring = "This page is linked from page with path ""/#iif(links.urlpath EQ "", DE('Home'),DE(links.urlpath))#.""<br>">
                </cfif>
            </cfif>
	...
	<cffunction name="DeletePage" output="false">
		...
		<cfset variables.request.notifyObservers("pages.deletepage", variables.itemdata)>
	...
	<cffunction name="publish">
        ...
        <cfset variables.request.notifyObservers("pages.publish", variables.itemdata)>
		...
		--->
		<!--- 1 --->
		<!--- page objects can be pseudo recursive for multispot --->
		<cfquery name="qs.clearnested" datasource="#variables.request.getvar('dsn')#" result="m">
			UPDATE  spo
			SET spo.deleted = 1
			FROM pageObjects po
			INNER JOIN pageObjects spo ON spo.pageid = po.id
			WHERE po.pageid = <cfqueryparam value="#variables.itemdata.id#" cfsqltype="cf_sql_varchar">
			AND po.status = 'published'
			AND po.deleted = 0
			AND po.siteid = <cfqueryparam value="#userobj.getCurrentSiteId()#" cfsqltype="cf_sql_varchar">	
			AND spo.status = 'published'
			AND spo.deleted = 0
			AND spo.siteid = <cfqueryparam value="#userobj.getCurrentSiteId()#" cfsqltype="cf_sql_varchar">	
		</cfquery>
		...
		<cfquery name="qs.getUnpublished" datasource="#variables.request.getvar('dsn')#">
			SELECT id, name, [module], title, data, pageid, siteid, status, deleted, memberType
			FROM pageObjects
			WHERE 
		...
		<cfquery name="qs.getUnpublishedsub" datasource="#variables.request.getvar('dsn')#">
			SELECT spo.id, spo.name, spo.[module], spo.title, spo.data, spo.pageid, spo.siteid, spo.status, spo.deleted, spo.memberType
			FROM pageObjects po
			INNER JOIN pageObjects spo ON po.id = spo.pageid
			WHERE 
				po.pageid = <cfqueryparam value="#variables.itemdata.id#" cfsqltype="cf_sql_varchar">
				AND po.status = 'staged'
				AND po.deleted = 0
				AND po.siteid = <cfqueryparam value="#userobj.getCurrentSiteId()#" cfsqltype="cf_sql_varchar">	
				AND spo.status = 'staged'
				AND spo.deleted = 0
				AND spo.siteid = <cfqueryparam value="#userobj.getCurrentSiteId()#" cfsqltype="cf_sql_varchar">	
		</cfquery>
		...
		<cfloop query="qs.getUnpublished">
			...
			<cfquery name="qs.rebublish" datasource="#variables.request.getvar('dsn')#">
				INSERT INTO pageObjects (
					id, name, title, [module], data, pageid, memberType, siteid, status
				) VALUES (
					...
					<cfqueryparam value="#qs.getUnpublished.title#" cfsqltype="cf_sql_varchar">,
			...
			<cfquery name="qs.publishedPageObjectsArchive" datasource="#variables.request.getvar('dsn')#">
				INSERT INTO publishedPageObjectsArchive (
					id, name, title, [module], data, pageid, memberType, siteid, publishedPageArchiveId
				) VALUES (
		...
		<cfloop query="qs.getUnpublishedsub">
				<cfif listfindnocase('simplecontent,htmlcontent', qs.getUnpublishedsub.module) 
						AND left(qs.getUnpublishedsub.data,2) NEQ "{}">
					<cfset qs.json = createobject('component', 'utilities.json')>
					<cfset qs.jsondata = qs.json.decode(qs.getUnpublishedsub.data)>
					<cfset qs.data =  qs.jsondata.content>
				<cfelse>
					<cfset qs.data = qs.getUnpublishedsub.data>
				</cfif>
				...
				<cfquery name="qs.rebublish" datasource="#variables.request.getvar('dsn')#">
					INSERT INTO pageObjects (
						...
						<cfqueryparam value="#qs.getUnpublishedsub.id#" cfsqltype="cf_sql_varchar">,
						<cfqueryparam value="#qs.getUnpublishedsub.name#" cfsqltype="cf_sql_varchar">,
						<cfqueryparam value="#qs.getUnpublishedsub.title#" cfsqltype="cf_sql_varchar">,
						<cfqueryparam value="#qs.getUnpublishedsub.module#" cfsqltype="cf_sql_varchar">,
						<cfqueryparam value="#qs.data#" cfsqltype="cf_sql_varchar">,
						<cfqueryparam value="#qs.getUnpublishedsub.pageid#" cfsqltype="cf_sql_varchar">,
	                    <cfqueryparam value="#qs.getUnpublished.memberType#" cfsqltype="cf_sql_varchar">,
						<cfqueryparam value="#userobj.getCurrentSiteId()#" cfsqltype="cf_sql_varchar">,
						'published'
					)
	...
	<cffunction name="deleteObjects">
		...
		<cfquery name="q2" datasource="#variables.request.getvar('dsn')#">
			DELETE FROM spo
			FROM pageObjects po
			INNER JOIN pageObjects spo ON spo.pageid = po.id
			WHERE po.pageid = <cfqueryparam value="#variables.itemData.id#" cfsqltype="cf_sql_varchar">
