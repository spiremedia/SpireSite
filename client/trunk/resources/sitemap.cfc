<cfcomponent name="sitemap" extends="resources.abstractmodel">
	...
	<cffunction name="getPageObjects">
		...
		<cfquery name="smpageobjects" datasource="#requestObject.getvar('dsn')#" result="m">
			SELECT id, [module], title, data, name, memberType, 'page' objtype,
			    CASE 
			...
			UNION ALL
			SELECT bpo.id, bpo.[module], bpo.title, bpo.data, bpo.name, bpo.memberType, 'block' objtype,
			    CASE 
				WHEN memberType = 'default' THEN 3 + sortorder
				ELSE 2 + sortorder
			    END as sort
			FROM <cfif ispreview>staged<cfelse>published</cfif>BlockPageObjects bpo
			WHERE 	(
					(bpo.location = <cfqueryparam value="/#arguments.path#" cfsqltype="CF_SQL_VARCHAR"> AND bpo.behavior = 'exact')
					OR (bpo.location = SUBSTRING ( <cfqueryparam value="/#arguments.path#%" cfsqltype="CF_SQL_VARCHAR"> , 1, LEN(bpo.location) ) AND bpo.behavior = 'below')
				)
				AND bpo.siteid = <cfqueryparam value="#variables.requestObject.getVar('siteid')#" cfsqltype="CF_SQL_VARCHAR">
				AND (
					bpo.memberType = <cfqueryparam value="#memberType#" cfsqltype="CF_SQL_VARCHAR">
					OR bpo.memberType = 'default'
				)
				AND bpo.active  = 1
		    ORDER BY sort
		</cfquery>
    ...
    <cffunction name="getSectionPages">
		...
		<cfif NOT (structkeyexists(arguments, "pageid") OR structkeyexists(arguments, "path")) >
			<cfthrow message="sitemap.sectionpages requires pageid or path">
		</cfif>
		...
		<cfquery name="childpages" datasource="#variables.requestObject.getVar('dsn')#" result="m">
			SELECT 
				...
				c.displayurlpath 
            FROM publishedpages c
			INNER JOIN publishedpages p ON (
				<cfif structkeyexists(arguments, "pageid")>p.id = <cfqueryparam value="#arguments.pageid#" cfsqltype="cf_sql_varchar"> 
				<cfelse>p.urlpath = <cfqueryparam value="#arguments.path#" cfsqltype="cf_sql_varchar">  </cfif>
				AND p.id = c.parentid )
			WHERE
	...
	<cffunction name="getSitePages">
		...
		<cfquery name="sitepages" datasource="#variables.requestObject.getVar('dsn')#">
			SELECT 
				...
				sort,
				parentid,
				len(displayurlpath) - len(replace(displayurlpath, '/','')) AS level,
				id
			FROM publishedPages 
