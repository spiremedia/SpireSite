<cfcomponent name="Module Model" output="false" extends="resources.abstractmodel">
	...
	<cffunction name="setup">
		...
		<cfset makeFile("configxml.cfm", "configxml.cfm", l.replaceable)>
	</cffunction>
	
	<cffunction name="hastemplatesdirectory">
		<cfreturn directoryexists(requestObj.getVar("machineroot") & "modules/" & variables.foldername & "/templates")>
	</cffunction>
	
	<cffunction name="hasmodelsdirectory">
		<cfreturn directoryexists(requestObj.getVar("machineroot") & "modules/" & variables.foldername & "/models")>
	</cffunction>
	
	<cffunction name="getControllerCode">
		<cfset var l = structnew()>
		<cfset l.ctrlerpath = requestObj.getVar("machineroot") & "modules/" & variables.foldername & "/controller.cfc">
	...
	<cffunction name="modelAlreadyExists">
		...
		<cfreturn fileexists(requestObj.getVar("machineroot") & "/modules/" & variables.foldername & "/models/" & name & ".cfc")>
	</cffunction>
	
	<cffunction name="testFileAlreadyExists">
		<cfargument name="name">
		<cfreturn fileexists(requestObj.getVar("machineroot") & "/modules/" & variables.foldername & "/" & name)>
	</cffunction>
	...
	<cffunction name="getunittests">
		...
		<cfdirectory action="list" name="uf" directory="#requestObj.getVar("machineroot") & "/modules/" & variables.foldername#" filter="*test.cfc">
	...
	<cffunction name="getmodels">
		...
		<cfdirectory action="list" name="uf" directory="#requestObj.getVar("machineroot") & "/modules/" & variables.foldername & "/models"#" filter="*.cfc">		
	...
	<cffunction name="gettemplates">
		...
		<cfdirectory action="list" name="uf" directory="#requestObj.getVar("machineroot") & "/modules/" & variables.foldername & "/templates"#" filter="*.cfm">	
	...
	<cffunction name="getsubcontrollers">
		<cfset var uf = "">
		<cfset var a = arraynew(1)>
		<cfset var a2 = arraynew(1)>
		<!--- <cfset var modulexml = getXML()> --->
		...
		<cfdirectory action="list" name="uf" directory="#requestObj.getVar("machineroot") & "/modules/" & variables.foldername#" filter="*ctrl.cfc">	
	...
	<cffunction name="subcontrollerexists">
		<cfargument name="filename">
		<cfreturn fileexists(requestObj.getVar("machineroot") & "/modules/" & variables.foldername & "/" & filename)>
	</cffunction>
	
	<cffunction name="templateexists">
		<cfargument name="filename">
		<cfreturn fileexists(requestObj.getVar("machineroot") & "/modules/" & variables.foldername & "/templates/" & filename)>
	</cffunction>
		
	<cffunction name="getxml">
		<cfset var modulexml = "">
		<cfinclude template="../../modules/#variables.foldername#/configxml.cfm">
		<cfreturn modulexml>
	</cffunction>		
	...
	<cffunction name="makefile">
		...
		<!--- get file contents --->
		<cffile action="read" variable="l.txt" file="#requestObj.getVar("machineRoot") & "/modules/moduleManager/patterns/" & patternfile & ".txt"#">

		<cfset l.txt = runReplaceables(l.txt, arguments.replaceable)>
				
		<cfset makefileFromString(targetFile, l.txt)>

	</cffunction>
	
	<cffunction name="makeFileFromString">
		<cfargument name="targetFile" required="true">
		<cfargument name="text" required="true">
		
		<cfif find("/",arguments.targetfile) AND NOT directoryexists(requestObj.getVar("machineroot") & "/modules/" & variables.foldername & "/" & gettoken(arguments.targetfile,1,"/"))>
			<cfdirectory action="create" directory="#requestObj.getVar("machineroot") & "/modules/" & variables.foldername & "/" & gettoken(arguments.targetfile,1,"/")#" mode="644">
		</cfif>
				
		<cffile action="write" file="#requestObj.getVar("machineroot") & "/modules/" & variables.foldername & "/" & targetFile#" output="#text#">
	</cffunction>
	...
	<cffunction name="renderTemplateCode">
		...
		<cffile action="read" variable="l.ptxt" file="#requestObj.getVar("machineRoot") & "/modules/modulemanager/patterns/" & patternfile & ".txt"#">
	...
	<cffunction name="addStringToFile">
		<cfargument name="targetFile" required="true">
		<cfargument name="strtoadd" required="true">
		<cfargument name="targetString" required="true">
		<cfargument name="targetinstance" required="false" default="afterfirst">
		
		<cfset var l = structnew()>

		<cffile action="read" variable="l.ttxt" file="#requestObj.getVar("machineRoot") & "/modules/" & variables.foldername & "/" & targetfile#">
		
		<cfset l.ttxt = insertStringIntoString(strtoadd, targetstring, l.ttxt, targetinstance)>
		
				
		<cffile action="write" file="#requestObj.getVar("machineRoot") & "/modules/" & variables.foldername & "/" & targetfile#" output="#trim(l.ttxt)#" mode="644">

	</cffunction>
	
	<cffunction name="insertStringIntoString">
		<cfargument name="stringtoadd" required="true">
		<cfargument name="targetstring" required="true">
		<cfargument name="wrapstring" required="true">
		<cfargument name="targetinstance" required="false" default="afterfirst">
		
		<cfset var l = structnew()>
		<cfset l.ptxt = stringtoadd>
		<cfset l.ttxt = wrapstring>
	...
	<cffunction name="makeModel">
		...
		<cfif tablename NEQ "">
			...
			<!--- <cfset ps = jsonObj.encode(ps)> --->
			<cfset replaceable.tablemetadata = "<cfset startOrm('" & ps & "')>">
	...
	<cffunction name="getFormItemString">
		...
		<cfset var tableMdl = createObject("component","resources.orm.start").init(requestObj,"activityLogs")>
		<cfset var ps = structnew()>
		<cfset var tmp = "">
		<cfset var repleaceable = structnew()>
		<cfset var tableInfo = tableMdl.getTableFields(tableName)>
