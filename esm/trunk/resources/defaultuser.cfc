<cfcomponent name="defaultuser" output="False">
	...
	<cffunction name="setFlash">
		<cfargument name="message" required="true">
		<cfparam name="variables.messages" default="#arraynew(1)#">
		<cfset arrayappend(variables.messages, message)>
	</cffunction>
	
	<cffunction name="getFlash">
		<cfset var temp = "">
		<cfif NOT structkeyexists(variables, "messages")>
			<cfreturn "">
		</cfif>
		<cfset temp = arraytolist(variables.messages, "<br>")>
	...
	<cffunction name="dumpUserInfo" output="False">
		<cfset var lcl = structnew()>
		<cfset lcl.id = "">
		<cfset lcl.username = "">
		<cfset lcl.fullName = "">
		<cfif structkeyexists(variables, 'id')>
			<cfset lcl.id = getUserID()>
		</cfif>
		<cfif structkeyexists(variables, 'username')>
			<cfset lcl.username = getUsername()>
		</cfif>
		<cfif structkeyexists(variables, 'fname') AND structkeyexists(variables, 'lname')>
			<cfset lcl.fullName = getFullName()>
		</cfif>
		<cfreturn lcl>
	</cffunction>
