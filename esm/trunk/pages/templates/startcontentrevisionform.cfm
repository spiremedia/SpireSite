...
<script>
	...
	function availableitems(s){
		...
		s+= "</ul>";
		gebid('step2').style.display = 'block';
		gebid('availableitemsdiv').innerHTML = s;
	}
	...
	function loadContentHTML(id){
		...
		gebid('step3').style.display = 'block';
		gebid('oldcontent').innerHTML = s.URL;
		gebid('oldcontent').innerHTML = 'Loading';
		gebid('step4').style.display = 'none';
		gebid('step5').style.display = 'none';
		gebid('step6').style.display = 'none';
		gebid('step4message').innerHTML = "";
		ajaxupdater(s);
	}
	function notrightcontent(){
		gebid('oldcontent').innerHTML = '';
		gebid('step3').style.display = 'none';
		gebid('step4').style.display = 'none';	
		gebid('step5').style.display = 'none';
		gebid('step6').style.display = 'none';
	}
	function isrightcontent(){
		gebid('step4').style.display = 'block';
	}
	function getTargetPageOptions(){
		...
		gebid('dump').innerHTML = '/pages/getRevertibleTargetList/?' + s;
		ajaxWResponseJsCaller('/pages/getRevertibleTargetList/', s);
	}
	function targetoptions(m){
		if (m.length == 0){
			gebid('step4message').innerHTML = 'There are no options available on that page that can accomodate this type of module.  For instance, if you want to revert a DHTML Pager, the target page must have a spot that can accept a DHTML Pager. ';
			gebid('targetRecipientList').innerHTML = "";
			return;
		}
		...
		gebid('step5').style.display = 'block';
		gebid('targetRecipientList').innerHTML = s;
		gebid('step4message').innerHTML = "";
	}
	function chooseTargetItem(spotName, memberType){
		...
		gebid('step6').style.display = 'block';
		return false;
	}
	...
	function revertcontentrslt(status){
		if (status)
			gebid('resultsMessage').innerHTML = "Success! <a href='/pages/editPage/?id=" + document.myForm.targetpageid.value + "'>Continue to the page</a>."
		else
			gebid('resultsMessage').innerHTML = "There was an issue. Either use the view screen to copy and paste your content or contact your technical department.";
	}
