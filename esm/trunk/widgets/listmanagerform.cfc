<cfcomponent name="tinymce">
	...
	<cffunction name="showform">
		...
		<cfif structkeyexists(variables.addtlinfo, "groupkey")>
			<cfoutput query="variables.options" group="groupkey">
				<cfset add("<optgroup label='#variables.options['groupvalue'][variables.options.currentrow]#'>")>
				<cfoutput>
		...
		<cfset add('<input type=''hidden'' name=''#variables.name#'' id=''#variables.id#'' value="">')>
