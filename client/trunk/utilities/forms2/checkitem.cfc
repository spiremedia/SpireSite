<cfcomponent name="form2text" extends="utilities.forms2.leaf">
	<cfthrow message="checkitem is deprecated for checkboxitem">
	<cffunction name="setChecked">
		<cfargument name="checked">
		<cfset variables.frominfo.checked = arguments.checked>
	</cffunction>
	
	<cffunction name="showHTML">
		<cfset var lcl = structnew()>

		<cfset addClassToWrapper("formitem_check")>
		...
		<cfsavecontent variable="lcl.h">
			...
			<input <cfif structkeyexists(lcl, "formclasses")>class="#lcl.formclasses#"</cfif>
				...
				<cfif structkeyexists(variables.formdata, variables.forminfo.name) AND variables.formdata[variables.forminfo.name] EQ variables.forminfo.default>checked</cfif>
