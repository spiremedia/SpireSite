<cfcomponent name="datavalidator">
	...
	<cffunction name="getFormattedErrors">
		...
		<cfif arraylen(variables.errors)>
			<cfsavecontent variable="errorstring">
				<cfoutput>
				<div class="formMessage">Please fix these problems to continue:</div>
	...
	<cffunction name="isvalidphone">
		<cfargument name="field">
		<cfargument name="val">
		<cfargument name="text">
		<cfif NOT IsValid("telephone", arguments.val)>
			<cfset addError(arguments.field, arguments.text)>
		</cfif>
	</cffunction>
	...
	<cffunction name="isvalidcreditcard">
		<cfargument name="field" required="true">
		<cfargument name="strCardNumber" required="true">
		<cfargument name="strCardType" required="true">
		<cfargument name="text">
		<cfscript>
			var rv = "";
			var str = "";
			...
			for (i=1; i lte arraylen(arrCCTestNumbers); i=i+1) {
				if (listfirst(arrCCTestNumbers[i],",") eq arguments.strCardNumber AND listlast(arrCCTestNumbers[i],",") eq arguments.strCardType) {
					return;
				}		
			}	
			
			if(reFind("[^[:digit:]]",arguments.strCardNumber)){
