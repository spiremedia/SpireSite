<cfcomponent name="site">
	...
	<cffunction name="getPage">
		...
		<cfset var path = arguments.requestObj.getformurlvar('path')>
		...
		<!--- notify  --->
		<cfif structisempty(page)>
			<cfset requestObj.notifyObservers("pagenotfound", path)>
		</cfif>
		...
		<cfreturn createObject('component', page.loadcfc).init(page,requestObj)>
	</cffunction>
